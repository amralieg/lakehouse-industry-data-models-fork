...
CREATE OR REPLACE TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim` (
    ...
    `inspection_order_id` BIGINT COMMENT 'Foreign key linking to coverage.inspection_order. Business justification: Pre-bind inspection reports establish baseline property condition for damage assessment, fraud detection, and pre-existing damage disputes.',
    ...
    `submission_id` BIGINT COMMENT 'Foreign key linking to coverage.submission. Business justification: Claims adjusters review original submission for underwriting history, pre-bind risk assessment, loss history verification, and fraud investigation.',
    `underwriting_risk_score_id` BIGINT COMMENT 'Foreign key linking to coverage.underwriting_risk_score. Business justification: Claims adjusters review underwriting risk scores for SIU referral decisions, subrogation potential assessment, and fraud pattern analysis.',
    ...
    `lob_description` STRING COMMENT 'Full description of the line of business for this claim.',
    ...
    `peril_code` STRING COMMENT 'Code representing the cause of loss or peril.',
    `peril_description` STRING COMMENT 'Full description of the cause of loss or peril.',
...
) COMMENT 'Claim: A reported loss or demand for indemnification under a policy. GRAIN: One row per reported loss. Grain: one row per reported loss.';
...
CREATE OR REPLACE TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` (
    ...
    `mailing_address_line1` STRING COMMENT 'First line of the claimants mailing address for correspondence and payment delivery.',
    `mailing_address_line2` STRING COMMENT 'Second line of the claimants mailing address, typically for apartment, suite, or unit number.',
    `mailing_city` STRING COMMENT 'City component of the claimants mailing address.',
    `mailing_country_code` STRING COMMENT 'Three-letter ISO country code for the claimants mailing address country.. Valid values are `USA|CAN|MEX`',
    `mailing_postal_code` STRING COMMENT 'Postal or ZIP code component of the claimants mailing address.',
    `mailing_state_province` STRING COMMENT 'State or province component of the claimants mailing address, using standard postal abbreviations.',
...
CREATE OR REPLACE TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_exposure` (
    ...
    `catastrophegeography_peril_id` BIGINT COMMENT 'Foreign key linking to catastrophegeography.peril. Business justification: Claim exposures track coverage_type and loss_cause but lack FK to peril master. Underwriters and actuaries need peril attributes (cat_peril flag, severity classification',
    ...
    `quote_coverage_id` BIGINT COMMENT 'Foreign key linking to coverage.quote_coverage. Business justification: Coverage disputes require tracing to quoted coverage terms to verify representations made during underwriting vs. bound policy.',
...
) COMMENT 'Claim Exposure: The coverage line within a claim, linking Claim to Coverage and Insured Risk. GRAIN: One row per coverage line per claim. Grain: one row per coverage line per claim.';
...
CREATE OR REPLACE TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_status` (
    ...
    `status_claim_id` BIGINT COMMENT 'Foreign key to the parent claim for which this status transition applies.',
    `transaction_claim_id` BIGINT COMMENT 'Unique transaction identifier from the source system for this status change event, enabling traceability and reconciliation.',
...
CREATE OR REPLACE TABLE `vibe_pc_insurance_blog_v499`.`claims`.`adjuster` (
    ...
    `license_expiration_date` DATE COMMENT 'Date the adjuster license expires and requires renewal.',
    `license_issue_date` DATE COMMENT 'Date the adjuster license was originally issued by the state authority.',
    `license_number` STRING COMMENT 'State-issued license number authorizing the adjuster to handle insurance claims.',
    `license_state` STRING COMMENT 'Two-letter state code where the adjuster holds their primary license.',
...
CREATE OR REPLACE TABLE `vibe_pc_insurance_blog_v499`.`claims`.`fnol` (
    ...
    `submission_id` BIGINT COMMENT 'Foreign key linking to coverage.submission. Business justification: FNOL intake for recently bound policies (especially binding authority) references submission for rapid policy verification, producer validation, and risk characteristic confirmation',
    ...
    `lob` STRING COMMENT 'Line of business for the policy under which the loss is reported (Personal Auto, Homeowners, Commercial General Liability, Workers Compensation, etc.).',
...
CREATE OR REPLACE TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` (
    `siu_referral_id` BIGINT COMMENT 'Unique identifier for the siu_referral data product (auto-inserted during validation).',
    `adjuster_id` BIGINT COMMENT 'Identifier of the SIU analyst or investigator assigned to the case.',
    `claim_id` BIGINT COMMENT 'Foreign key to the claim being referred for fraud investigation.',
    `referring_party_id` BIGINT COMMENT 'Identifier of the person or system that initiated the SIU referral.',
    `assignment_date` DATE COMMENT 'Date the referral was assigned to an investigator.',
    `case_number` STRING COMMENT 'Case number assigned by law enforcement agency for tracking prosecution.',
    `clue_report_ordered_flag` BOOLEAN COMMENT 'Indicates whether a CLUE report was ordered as part of the investigation.',
    `confirmed_fraud_amount` DECIMAL(15,2) COMMENT 'Confirmed dollar value of fraud identified after investigation completion.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the SIU referral record was first created in the system.',
    `disposition` STRING COMMENT 'Action taken or recommended based on the investigation outcome. [ENUM-REF-CANDIDATE: claim_denied|claim_adjusted|claim_paid|prosecution_recommended|civil_action|no_action|pending_review — 7 candidates stripped; promote to reference product]',
    `estimated_fraud_amount` DECIMAL(15,2) COMMENT 'Estimated dollar value of the suspected fraudulent claim or loss.',
    `evidence_summary` STRING COMMENT 'Summary of evidence collected during the investigation supporting the outcome.',
    `external_database_check_flag` BOOLEAN COMMENT 'Indicates whether external databases such as CLUE or ISO were queried during investigation.',
    `fraud_indicator_code` STRING COMMENT 'Primary fraud indicator or red flag code that triggered the referral.',
    `fraud_indicator_description` STRING COMMENT 'Detailed description of the fraud indicator or suspicious activity observed.',
    `fraud_scheme_category` STRING COMMENT 'Category of fraud scheme suspected, such as staged accident, inflated claim, or false injury. [ENUM-REF-CANDIDATE: staged_accident|inflated_claim|false_injury|phantom_treatment|identity_theft|arson|premium_fraud|application_fraud — promote to reference',
    `fraud_type` STRING COMMENT 'Classification of the suspected fraud type based on nature and severity.. Valid values are `hard_fraud|soft_fraud|opportunistic|organized|internal|external`',
    `investigation_close_date` DATE COMMENT 'Date the SIU investigation was completed and closed.',
    `investigation_notes` STRING COMMENT 'Detailed notes and findings from the SIU investigation process.',
    `investigation_outcome` STRING COMMENT 'Final determination of the SIU investigation regarding fraud presence.. Valid values are `confirmed_fraud|suspected_fraud|cleared|inconclusive|withdrawn|referred_to_law_enforcement`',
    `investigation_start_date` DATE COMMENT 'Date the formal SIU investigation commenced.',
    `iso_claim_search_flag` BOOLEAN COMMENT 'Indicates whether an ISO ClaimSearch was performed during the investigation.',
    `law_enforcement_agency` STRING COMMENT 'Name of the law enforcement agency to which the case was referred.',
    `law_enforcement_referral_date` DATE COMMENT 'Date the case was referred to law enforcement authorities.',
    `law_enforcement_referral_flag` BOOLEAN COMMENT 'Indicates whether the case was referred to law enforcement for criminal prosecution.',
    `naic_fraud_category_code` STRING COMMENT 'NAIC standardized code for fraud category used in regulatory reporting.',
    `nicb_referral_flag` BOOLEAN COMMENT 'Indicates whether the case was referred to NICB for industry-wide fraud tracking.',
    `policy_cancellation_recommended_flag` BOOLEAN COMMENT 'Indicates whether policy cancellation was recommended based on fraud findings.',
    `priority_level` STRING COMMENT 'Priority assigned to the referral based on severity and potential financial impact.. Valid values are `low|medium|high|critical`',
    `recovery_action_taken` STRING COMMENT 'Type of recovery action pursued to recoup fraudulent payments or losses.. Valid values are `none|subrogation|restitution|civil_suit|settlement|claim_denial`',
    `referral_date` DATE COMMENT 'Date the claim was referred to SIU for fraud investigation.',
    `referral_number` STRING COMMENT 'Business identifier for the SIU referral, used for tracking and reporting.',
    `referral_source` STRING COMMENT 'Origin or channel through which the fraud suspicion was identified and referred. [ENUM-REF-CANDIDATE: adjuster|underwriter|automated_system|tip_line|claimant_interview|medical_review|surveillance|external_agency — 8 candidates stripped; promote to',
    `referral_status` STRING COMMENT 'Current status of the SIU referral in the investigation lifecycle.. Valid values are `pending|under_investigation|closed_confirmed_fraud|closed_cleared|closed_insufficient_evidence|withdrawn`',
    `referral_timestamp` TIMESTAMP COMMENT 'Precise timestamp when the referral was created in the system.',
    `referring_party_role` STRING COMMENT 'Role of the party who made the referral, such as adjuster, underwriter, or system.',
    `savings_amount` DECIMAL(15,2) COMMENT 'Dollar amount saved by the insurer through fraud detection and prevention.',
    `statement_obtained_flag` BOOLEAN COMMENT 'Indicates whether a formal statement was obtained from the claimant or suspect.',
    `surveillance_conducted_flag` BOOLEAN COMMENT 'Indicates whether physical or digital surveillance was conducted during investigation.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when the SIU referral record was last modified.',
    CONSTRAINT pk_siu_referral PRIMARY KEY(`siu_referral_id`)
) COMMENT 'Special Investigations Unit referral for a claim suspected of fraud. Grain: one row per referral. Captures referral reason, SIU analyst assigned, investigation outcome, and disposition (confirmed fraud, cleared, pending). FK to claim.';

CREATE OR REPLACE TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` (
    `claim_note_id` BIGINT COMMENT 'Unique identifier for the claim note entry.',
    `claim_author_party_id` BIGINT COMMENT 'Reference to the party who authored or created the note, typically an adjuster, examiner, or system user.',
    `claim_exposure_id` BIGINT COMMENT 'Reference to the specific claim exposure or coverage line this note pertains to, if applicable.',
    `claim_id` BIGINT COMMENT 'Reference to the parent claim this note is attached to.',
    `claim_last_modified_by_party_id` BIGINT COMMENT 'Reference to the party who last modified or edited the note.',
    `related_document_claim_document_id` BIGINT COMMENT 'Reference to an associated document, image, or attachment linked to this note.',
    `activity_code` STRING COMMENT 'Standardized code representing the claims activity or action associated with this note.',
    `attorney_client_privilege_flag` BOOLEAN COMMENT 'Indicator whether the note is protected under attorney-client privilege and exempt from discovery.',
    `author_name` STRING COMMENT 'Display name of the individual who authored the note.',
    `author_role` STRING COMMENT 'Functional role of the author at the time of note creation, such as adjuster, supervisor, examiner, or legal counsel.',
    `confidential_flag` BOOLEAN COMMENT 'Indicator whether the note contains confidential or sensitive information requiring restricted access.',
    `coverage_analysis_flag` BOOLEAN COMMENT 'Indicator whether the note contains coverage analysis, interpretation, or determination commentary.',
    `diary_completed_flag` BOOLEAN COMMENT 'Indicator whether the diary action has been completed and closed.',
    `diary_completed_timestamp` TIMESTAMP COMMENT 'Date and time when the diary action was marked as completed.',
    `diary_due_date` DATE COMMENT 'Target date by which the diary note requires follow-up action or review.',
    `diary_flag` BOOLEAN COMMENT 'Indicator whether the note is a diary entry requiring follow-up action or review.',
    `entry_timestamp` TIMESTAMP COMMENT 'Date and time when the note was created or entered into the system.',
    `external_communication_flag` BOOLEAN COMMENT 'Indicator whether the note documents communication with external parties such as claimants, attorneys, or vendors.',
    `fraud_indicator_flag` BOOLEAN COMMENT 'Indicator whether the note contains observations or evidence related to potential fraud or Special Investigation Unit (SIU) referral.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Date and time when the note was last updated or edited.',
    `liability_analysis_flag` BOOLEAN COMMENT 'Indicator whether the note contains liability assessment or fault determination analysis.',
    `loss_description_flag` BOOLEAN COMMENT 'Indicator whether the note contains a description of the loss event or circumstances.',
    `note_category` STRING COMMENT 'Broader categorical grouping of the note for reporting and filtering purposes.',
    `note_number` STRING COMMENT 'Business-facing sequential or formatted identifier for the note within the claim.',
    `note_source_system` STRING COMMENT 'Originating system or application where the note was created, such as ClaimCenter, manual entry, or mobile app.',
    `note_status` STRING COMMENT 'Current lifecycle status of the note: active, archived, deleted, or superseded by a newer entry.. Valid values are `ACTIVE|ARCHIVED|DELETED|SUPERSEDED`',
    `note_subject` STRING COMMENT 'Brief subject or title summarizing the content of the note.',
    `note_text` STRING COMMENT 'Full text content of the note entry, capturing detailed observations, actions, or commentary.',
    `note_type_code` STRING COMMENT 'Classification of the note by functional purpose: diary, coverage analysis, legal, medical, investigation, general, reserve, payment, recovery, or subrogation.',
    `regulatory_reportable_flag` BOOLEAN COMMENT 'Indicator whether the note content is relevant for regulatory reporting or market conduct examination purposes.',
    `reserve_recommendation_flag` BOOLEAN COMMENT 'Indicator whether the note contains a reserve recommendation or reserve change justification.',
    `sequence_number` BIGINT COMMENT 'Sequential ordering number of the note within the claim or claim exposure for chronological sorting.',
    `settlement_discussion_flag` BOOLEAN COMMENT 'Indicator whether the note documents settlement negotiations, offers, or discussions.',
    `visibility_classification` STRING COMMENT 'Access control classification determining who can view the note: public, internal, confidential, restricted, or legal privileged.. Valid values are `PUBLIC|INTERNAL|CONFIDENTIAL|RESTRICTED|LEGAL_PRIVILEGED`',
    `work_product_flag` BOOLEAN COMMENT 'Indicator whether the note constitutes attorney work product and is protected from disclosure.',
    CONSTRAINT pk_claim_note PRIMARY KEY(`claim_note_id`)
) COMMENT 'Diary and activity notes recorded against a claim or claim exposure. Grain: one row per note entry. Captures author, note type (diary, coverage, legal, medical), entry timestamp, and visibility classification.';

CREATE OR REPLACE TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` (
    `claim_document_id` BIGINT COMMENT 'Unique identifier for the claim document record.',
    `claim_exposure_id` BIGINT COMMENT 'Reference to the specific claim exposure or coverage line this document supports, if applicable.',
    `claim_id` BIGINT COMMENT 'Reference to the parent claim to which this document is attached.',
    `source_party_id` BIGINT COMMENT 'Reference to the party record representing the individual or organization that submitted the document.',
    `access_level` STRING COMMENT 'Security classification determining who may view or access the document within the organization.. Valid values are `public|internal|restricted|confidential`',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time the document record was first created in the claims management system.',
    `document_category` STRING COMMENT 'High-level grouping of the document for organizational and workflow purposes. [ENUM-REF-CANDIDATE: evidence|correspondence|financial|legal|medical|investigative|administrative — 7 candidates stripped; promote to reference product]',
    `document_date` DATE COMMENT 'Date the document was originally created, issued, or signed by the source party.',
    `document_description` STRING COMMENT 'Detailed narrative describing the contents, purpose, and relevance of the document to the claim.',
    `document_number` STRING COMMENT 'Business-assigned unique document number or reference code for tracking and retrieval.',
    `document_status` STRING COMMENT 'Current lifecycle state of the document within the claims adjudication workflow. [ENUM-REF-CANDIDATE: received|pending_review|reviewed|approved|rejected|archived|missing — 7 candidates stripped; promote to reference product]',
    `document_title` STRING COMMENT 'Descriptive title or name of the document for identification and indexing.',
    `document_type` STRING COMMENT 'Classification of the document by its purpose and content within the claims process. [ENUM-REF-CANDIDATE: police_report|medical_record|repair_estimate|photo|video|demand_letter|proof_of_loss|adjuster_report|witness_statement|appraisal — 10 candidates',
    `file_format` STRING COMMENT 'Technical file format or extension of the stored document. [ENUM-REF-CANDIDATE: pdf|jpg|jpeg|png|tiff|doc|docx|xls|xlsx|txt|xml|mp4|avi — 13 candidates stripped; promote to reference product]',
    `file_name` STRING COMMENT 'Original file name of the uploaded or scanned document including extension.',
    `file_size_bytes` BIGINT COMMENT 'Size of the document file measured in bytes for storage and transmission tracking.',
    `is_confidential` BOOLEAN COMMENT 'Indicator whether the document contains confidential or sensitive information requiring restricted access.',
    `is_required` BOOLEAN COMMENT 'Indicator whether the document is mandatory for claim adjudication or settlement per policy or regulatory requirements.',
    `is_verified` BOOLEAN COMMENT 'Indicator whether the authenticity and accuracy of the document have been validated by the adjuster or third party.',
    `notes` STRING COMMENT 'Free-form notes or comments from adjusters or handlers regarding the document content, quality, or relevance.',
    `page_count` BIGINT COMMENT 'Total number of pages in the document, applicable to multi-page formats.',
    `received_date` DATE COMMENT 'Date the document was received by the claims department or system.',
    `received_timestamp` TIMESTAMP COMMENT 'Precise date and time the document was received and logged into the claims management system.',
    `retention_expiration_date` DATE COMMENT 'Date after which the document may be purged or archived per retention policy.',
    `retention_period_years` BIGINT COMMENT 'Number of years the document must be retained per regulatory, legal, or company policy requirements.',
    `reviewed_by_user_code` BIGINT COMMENT 'Reference to the user or adjuster who reviewed and validated the document.',
    `reviewed_date` DATE COMMENT 'Date the document was reviewed and evaluated by the adjuster or claims handler.',
    `source_name` STRING COMMENT 'Name of the individual, organization, or system that provided the document.',
    `source_type` STRING COMMENT 'Classification of the party or entity that provided or originated the document. [ENUM-REF-CANDIDATE: claimant|insured|third_party|adjuster|medical_provider|repair_shop|law_enforcement|attorney|expert|internal — 10 candidates stripped; promote to reference',
    `storage_location` STRING COMMENT 'Physical or logical storage path, URI, or reference to the document repository where the file is stored.',
    `storage_system` STRING COMMENT 'Name or type of the document management or storage system housing the document.. Valid values are `dms|ecm|cloud_storage|file_server|archive|imaging_system`',
    `updated_timestamp` TIMESTAMP COMMENT 'Date and time the document record was last modified or updated in the system.',
    `verification_method` STRING COMMENT 'Method or process used to verify the authenticity and accuracy of the document.. Valid values are `manual_review|third_party_validation|digital_signature|notarization|cross_reference|none`',
    CONSTRAINT pk_claim_document PRIMARY KEY(`claim_document_id`)
) COMMENT 'Documents attached to a claim: police reports, medical records, repair estimates, photos, demand letters. Grain: one row per document. Captures document type, source, received date, and storage reference.';

CREATE OR REPLACE TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` (
    `medical_bill_id` BIGINT COMMENT 'Unique identifier for the medical bill record.',
    `adjuster_id` BIGINT COMMENT 'Foreign key to the adjuster who reviewed and processed this medical bill.',
    `claim_exposure_id` BIGINT COMMENT 'Foreign key to the claim exposure this bill is submitted against.',
    `claim_id` BIGINT COMMENT 'Foreign key to the parent claim for reporting and aggregation.',
    `claimant_id` BIGINT COMMENT 'Foreign key to the claimant who received the medical services.',
    `currency_id` BIGINT COMMENT 'Three-letter ISO 4217 currency code for all monetary amounts on this bill.',
    `original_bill_id` BIGINT COMMENT 'Foreign key to the original bill if this is a duplicate or corrected resubmission.',
    `party_id` BIGINT COMMENT 'Foreign key to the medical provider or facility that submitted the bill.',
    `allowed_amount` DECIMAL(15,2) COMMENT 'Amount allowed per fee schedule, contract, or usual and customary rates.',
    `bill_date` DATE COMMENT 'Date the provider issued the medical bill.',
    `bill_number` STRING COMMENT 'Provider-assigned unique bill or invoice number.',
    `bill_received_date` DATE COMMENT 'Date the medical bill was received by the insurer.',
    `bill_review_notes` STRING COMMENT 'Free-text notes from the bill review process explaining adjustments or denials.',
    `bill_review_outcome` STRING COMMENT 'Outcome of the bill review process by medical review or utilization review team.. Valid values are `approved|reduced|denied|pending|not_reviewed`',
    `bill_source` STRING COMMENT 'Origin or channel through which the medical bill was received.. Valid values are `provider_direct|claimant_submitted|attorney_submitted|third_party_administrator|electronic_data_interchange`',
    `bill_status` STRING COMMENT 'Current processing status of the medical bill in the claims workflow. [ENUM-REF-CANDIDATE: received|under_review|approved|partially_approved|denied|paid|appealed — 7 candidates stripped; promote to reference product]',
    `bill_type` STRING COMMENT 'Classification of the medical bill by service type. [ENUM-REF-CANDIDATE: hospital_inpatient|hospital_outpatient|physician|pharmacy|durable_medical_equipment|ambulance|other — 7 candidates stripped; promote to reference product]',
    `billed_amount` DECIMAL(15,2) COMMENT 'Total amount billed by the provider before any adjustments or reductions.',
    `coinsurance_amount` DECIMAL(15,2) COMMENT 'Percentage-based coinsurance amount owed by the claimant per policy terms.',
    `copay_amount` DECIMAL(15,2) COMMENT 'Fixed copayment amount owed by the claimant per policy terms.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this medical bill record was first created in the system.',
    `deductible_amount` DECIMAL(15,2) COMMENT 'Portion of the bill applied to the claimant deductible.',
    `denial_reason_code` STRING COMMENT 'Standardized code indicating the reason for bill denial or reduction.',
    `denial_reason_description` STRING COMMENT 'Human-readable description of the reason for bill denial or reduction.',
    `diagnosis_code_primary` STRING COMMENT 'Primary ICD-10 diagnosis code for the condition being treated.. Valid values are `^[A-Z][0-9]{2}(.[0-9]{1,4})?$`',
    `diagnosis_code_secondary` STRING COMMENT 'Comma-separated list of secondary ICD-10 diagnosis codes.',
    `is_duplicate` BOOLEAN COMMENT 'Indicates whether this bill is a duplicate of a previously submitted bill.',
    `paid_amount` DECIMAL(15,2) COMMENT 'Actual amount paid by the insurer to the provider or claimant.',
    `patient_dob` DATE COMMENT 'Date of birth of the patient for identification and eligibility verification.',
    `patient_name` STRING COMMENT 'Full name of the patient who received the medical services.',
    `payment_date` DATE COMMENT 'Date the insurer issued payment for this medical bill.',
    `payment_method` STRING COMMENT 'Method used to pay the medical bill to the provider or claimant.. Valid values are `check|eft|wire|card|other`',
    `payment_reference_number` STRING COMMENT 'Check number, EFT trace number, or other payment instrument identifier.',
    `procedure_code_primary` STRING COMMENT 'Primary CPT or HCPCS procedure code for the service rendered.. Valid values are `^[0-9]{5}$`',
    `procedure_code_secondary` STRING COMMENT 'Comma-separated list of secondary CPT or HCPCS procedure codes.',
    `provider_name` STRING COMMENT 'Name of the medical provider or facility as shown on the bill.',
    `provider_npi` STRING COMMENT 'Ten-digit National Provider Identifier for the billing provider.. Valid values are `^[0-9]{10}$`',
    `provider_tax_number` STRING COMMENT 'Federal Employer Identification Number of the billing provider.. Valid values are `^[0-9]{2}-[0-9]{7}$`',
    `reduction_amount` DECIMAL(15,2) COMMENT 'Amount reduced from billed to allowed due to fee schedule or contract negotiation.',
    `service_from_date` DATE COMMENT 'Start date of the medical service period covered by this bill.',
    `service_to_date` DATE COMMENT 'End date of the medical service period covered by this bill.',
...
) COMMENT 'Medical bill submitted against a bodily injury or workers comp claim exposure. Grain: one row per bill. Captures provider, procedure codes, billed amount, allowed amount, paid amount, and bill review outcome.';

CREATE OR REPLACE TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` (
    `repair_estimate_id` BIGINT COMMENT 'Unique identifier for the repair estimate record.',
    `approved_by_adjuster_id` BIGINT COMMENT 'Identifier of the adjuster who approved this repair estimate.',
    `claim_exposure_id` BIGINT COMMENT 'Links this estimate to the specific claim exposure (coverage line) being repaired.',
    `claim_id` BIGINT COMMENT 'Parent claim for which this repair estimate was prepared.',
    `currency_id` BIGINT COMMENT 'Three-letter ISO 4217 currency code for all monetary amounts in this estimate.',
    `estimator_party_id` BIGINT COMMENT 'Foreign key linking to party.party. Business justification: Estimators/appraisers are licensed parties (individuals or organizations) tracked for quality control, licensing verification, and vendor performance management.',
    `prior_estimate_repair_estimate_id` BIGINT COMMENT 'Reference to the previous estimate that this supplement or revision supersedes.',
    `agreed_amount` DECIMAL(15,2) COMMENT 'Final negotiated and agreed repair cost between insurer, insured, and repair facility, in policy currency.',
    `approval_date` DATE COMMENT 'Date the estimate was approved by the adjuster or claims handler.',
    `betterment_amount` DECIMAL(15,2) COMMENT 'Amount deducted from the estimate for betterment (improvement over pre-loss condition), in policy currency.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this repair estimate record was first created in the system.',
    `damage_description` STRING COMMENT 'Detailed narrative description of the damage observed during inspection, including scope and severity.',
    `deductible_applied_amount` DECIMAL(15,2) COMMENT 'Policy deductible amount applied to this repair estimate, in policy currency.',
    `depreciation_amount` DECIMAL(15,2) COMMENT 'Amount deducted for depreciation of parts or materials (Actual Cash Value adjustment), in policy currency.',
    `document_reference` STRING COMMENT 'Reference identifier or URI to the stored estimate document (PDF, image, or system file).',
    `estimate_date` DATE COMMENT 'Date the repair estimate was prepared or issued by the estimator or repair facility.',
    `estimate_method` STRING COMMENT 'Method used to prepare the estimate: manual, software-assisted, photo-based, AI-driven, or field inspection.. Valid values are `manual|software_assisted|photo_estimate|ai_estimate|field_inspection`',
    `estimate_notes` STRING COMMENT 'Additional notes, comments, or special instructions related to the repair estimate.',
    `estimate_number` STRING COMMENT 'Business identifier for the repair estimate, often assigned by the estimator or repair facility.',
    `estimate_status` STRING COMMENT 'Current lifecycle status of the repair estimate in the adjudication workflow.. Valid values are `draft|submitted|under_review|approved|rejected|superseded`',
    `estimate_type` STRING COMMENT 'Classification of the estimate: initial assessment, supplement for additional damage, revised, final agreed value, desk review, or field inspection.. Valid values are `initial|supplement|revised|final|desk_review|field_inspection`',
    `estimating_software` STRING COMMENT 'Name and version of the estimating software used (e.g., CCC ONE, Mitchell, Xactimate).',
    `estimator_type` STRING COMMENT 'Classification of who prepared the estimate: staff adjuster, independent adjuster, repair shop, third-party appraiser, or insured self-estimate.. Valid values are `staff_adjuster|independent_adjuster|repair_shop|third_party_appraiser|insured_estimate`',
    `inspection_date` DATE COMMENT 'Date the physical inspection of the damaged property or vehicle was conducted.',
    `labor_cost_amount` DECIMAL(15,2) COMMENT 'Total labor cost for the repair work, in policy currency.',
    `labor_hours` DECIMAL(8,2) COMMENT 'Total estimated labor hours required to complete the repair work.',
    `labor_rate_per_hour` DECIMAL(8,2) COMMENT 'Hourly labor rate charged by the repair facility, in policy currency.',
    `net_payable_amount` DECIMAL(15,2) COMMENT 'Net amount payable by the insurer after deductible, depreciation, and betterment adjustments, in policy currency.',
    `paint_materials_cost_amount` DECIMAL(15,2) COMMENT 'Cost of paint, finishing materials, and supplies for auto or property restoration, in policy currency.',
    `parts_cost_amount` DECIMAL(15,2) COMMENT 'Total cost of parts and materials required for the repair, in policy currency.',
    `parts_source` STRING COMMENT 'Classification of parts to be used: OEM (Original Equipment Manufacturer), aftermarket, recycled, refurbished, or used.. Valid values are `oem|aftermarket|recycled|refurbished|used`',
    `photo_count` BIGINT COMMENT 'Number of photos or images attached to document the damage and support the estimate.',
    `repair_facility_address` STRING COMMENT 'Physical address of the repair facility or contractor location.',
    `repair_facility_name` STRING COMMENT 'Name of the repair shop, body shop, contractor, or service provider that will perform or bid on the repairs.',
    `repair_facility_phone` STRING COMMENT 'Primary contact phone number for the repair facility or contractor.',
    `repair_facility_type` STRING COMMENT 'Classification of the repair facility: network shop, preferred vendor, OEM certified, independent, contractor, or restoration company.. Valid values are `network_shop|preferred_vendor|oem_certified|independent_shop|contractor|restoration_company`',
    `repair_scope_description` STRING COMMENT 'Detailed description of the proposed repair work, including materials, labor, and methods.',
    `sublet_cost_amount` DECIMAL(15,2) COMMENT 'Cost of subcontracted or outsourced work (e.g., glass, upholstery, specialty trades), in policy currency.',
    `supplement_number` BIGINT COMMENT 'Sequential number indicating this is the Nth supplement to the original estimate (0 for initial estimate).',
    `supplement_reason` STRING COMMENT 'Explanation for why a supplemental estimate was required (e.g., hidden damage discovered, scope change, price adjustment).',
    `tax_amount` DECIMAL(15,2) COMMENT 'Sales tax or other applicable taxes on the repair estimate, in policy currency.',
    `total_estimate_amount` DECIMAL(15,2) COMMENT 'Total estimated cost of the repair including parts, labor, materials, sublet, and taxes, in policy currency.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this repair estimate record was last modified.',
    CONSTRAINT pk_repair_estimate PRIMARY KEY(`repair_estimate_id`)
) COMMENT 'Property or auto repair estimate associated with a claim exposure. Grain: one row per estimate. Captures estimator, repair facility, estimate date, line-item costs, agreed value, and supplement history.';

CREATE OR REPLACE TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` (
    `salvage_disposition_id` BIGINT COMMENT 'Unique identifier for the salvage disposition record.',
    `adjuster_id` BIGINT COMMENT 'Identifier of the claims adjuster responsible for managing this salvage or subrogation disposition.',
    `claim_exposure_id` BIGINT COMMENT 'Links this salvage disposition to the specific claim exposure coverage line.',
    `claim_id` BIGINT COMMENT 'Links this salvage disposition to the parent claim record.',
    `currency_id` BIGINT COMMENT 'Three-letter ISO 4217 currency code for all monetary amounts in this disposition record.',
    `party_id` BIGINT COMMENT 'Identifier of the salvage vendor, auction house, or third-party recovery agent handling the disposition.',
    `salvage_party_id` BIGINT COMMENT 'Identifier of the attorney handling subrogation litigation if legal action is involved.',
    `salvage_subrogation_target_party_id` BIGINT COMMENT 'Identifier of the liable third party from whom subrogation recovery is being pursued.',
    `abandonment_reason` STRING COMMENT 'Reason for abandoning the salvage or subrogation pursuit if status is abandoned.',
    `actual_salvage_value` DECIMAL(15,2) COMMENT 'Actual amount recovered from the sale or disposition of salvaged property.',
    `buyer_name` STRING COMMENT 'Name of the buyer or entity that purchased the salvaged property.',
    `closed_date` DATE COMMENT 'Date when the salvage or subrogation pursuit was closed or finalized.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this salvage disposition record was first created in the system.',
    `disposition_notes` STRING COMMENT 'Free-text notes documenting key events, decisions, and outcomes in the salvage or subrogation pursuit.',
    `disposition_number` STRING COMMENT 'Business identifier for this salvage or subrogation disposition record.',
    `disposition_status` STRING COMMENT 'Current lifecycle status of the salvage or subrogation pursuit.. Valid values are `open|pending|closed|abandoned|settled|in_litigation`',
    `estimated_salvage_value` DECIMAL(15,2) COMMENT 'Estimated market value of the salvaged property at the time of disposition opening.',
    `litigation_filed_date` DATE COMMENT 'Date when legal action was filed for subrogation recovery.',
    `litigation_status` STRING COMMENT 'Current status of any legal action associated with subrogation pursuit.. Valid values are `not_applicable|pending|filed|settled|judgment|dismissed`',
    `net_recovery_amount` DECIMAL(15,2) COMMENT 'Net amount recovered after deducting all recovery expenses from gross salvage or subrogation proceeds.',
    `opened_date` DATE COMMENT 'Date when the salvage or subrogation pursuit was initiated.',
    `recovery_expenses` DECIMAL(15,2) COMMENT 'Total expenses incurred in pursuing salvage or subrogation recovery, including legal fees, vendor fees, and administrative costs.',
    `recovery_type` STRING COMMENT 'Type of recovery pursuit: salvage from damaged property, subrogation from liable third party, reinsurance recovery, or other.. Valid values are `salvage|subrogation|reinsurance|other`',
    `sale_date` DATE COMMENT 'Date when the salvaged property was sold or disposed of.',
    `sale_method` STRING COMMENT 'Method used to dispose of salvaged property: auction, private sale, scrap, donation, retained by insured, or other.. Valid values are `auction|private_sale|scrap|donation|retained|other`',
    `salvage_item_description` STRING COMMENT 'Detailed description of the salvaged property or damaged goods being recovered.',
    `salvage_item_type` STRING COMMENT 'Category of salvaged item: vehicle total loss, building materials, equipment, inventory, personal property, or other.. Valid values are `vehicle|building_materials|equipment|inventory|personal_property|other`',
    `settlement_date` DATE COMMENT 'Date when subrogation settlement was reached with the liable third party.',
    `storage_end_date` DATE COMMENT 'Date when salvaged property was removed from storage for sale or disposition.',
    `storage_location` STRING COMMENT 'Physical location where salvaged property is stored pending disposition.',
    `storage_start_date` DATE COMMENT 'Date when salvaged property was placed into storage.',
    `subrogation_demand_amount` DECIMAL(15,2) COMMENT 'Total amount demanded from the liable third party in subrogation pursuit.',
    `subrogation_recovered_amount` DECIMAL(15,2) COMMENT 'Actual amount recovered through subrogation from the liable third party.',
    `subrogation_target_name` STRING COMMENT 'Name of the liable third party or entity against whom subrogation is pursued.',
    `title_status` STRING COMMENT 'Legal title status of salvaged vehicle or property: clean, salvage, rebuilt, junk, pending transfer, or not applicable.. Valid values are `clean|salvage|rebuilt|junk|pending|not_applicable`',
    `title_transfer_date` DATE COMMENT 'Date when legal title of salvaged property was transferred to the insurer or buyer.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this salvage disposition record was last modified.',
    `vehicle_vin` STRING COMMENT '17-character VIN for salvaged vehicle if recovery involves an auto total loss.. Valid values are `^[A-HJ-NPR-Z0-9]{17}$`',
    `vendor_name` STRING COMMENT 'Name of the salvage vendor, auction house, or recovery agent managing the salvage sale or subrogation.',
    CONSTRAINT pk_salvage_disposition PRIMARY KEY(`salvage_disposition_id`)
) COMMENT 'Recovery-pursuit record per claim exposure covering both salvage disposition (auto total-loss, damaged goods) and subrogation against liable third parties. Grain: one row per recovery item.';
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_status` ADD CONSTRAINT `fk_claims_claim_status_status_claim_id` FOREIGN KEY (`status_claim_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claim`(`claim_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_status` ADD CONSTRAINT `fk_claims_claim_status_transaction_claim_id` FOREIGN KEY (`transaction_claim_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claim`(`claim_id`);
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ADD CONSTRAINT `fk_claims_siu_referral_adjuster_id` FOREIGN KEY (`adjuster_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`adjuster`(`adjuster_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ADD CONSTRAINT `fk_claims_siu_referral_claim_id` FOREIGN KEY (`claim_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claim`(`claim_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ADD CONSTRAINT `fk_claims_claim_note_claim_exposure_id` FOREIGN KEY (`claim_exposure_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claim_exposure`(`claim_exposure_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ADD CONSTRAINT `fk_claims_claim_note_claim_id` FOREIGN KEY (`claim_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claim`(`claim_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ADD CONSTRAINT `fk_claims_claim_note_related_document_claim_document_id` FOREIGN KEY (`related_document_claim_document_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claim_document`(`claim_document_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ADD CONSTRAINT `fk_claims_claim_document_claim_exposure_id` FOREIGN KEY (`claim_exposure_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claim_exposure`(`claim_exposure_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ADD CONSTRAINT `fk_claims_claim_document_claim_id` FOREIGN KEY (`claim_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claim`(`claim_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ADD CONSTRAINT `fk_claims_medical_bill_adjuster_id` FOREIGN KEY (`adjuster_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`adjuster`(`adjuster_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ADD CONSTRAINT `fk_claims_medical_bill_claim_exposure_id` FOREIGN KEY (`claim_exposure_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claim_exposure`(`claim_exposure_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ADD CONSTRAINT `fk_claims_medical_bill_claim_id` FOREIGN KEY (`claim_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claim`(`claim_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ADD CONSTRAINT `fk_claims_medical_bill_claimant_id` FOREIGN KEY (`claimant_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claimant`(`claimant_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ADD CONSTRAINT `fk_claims_medical_bill_original_bill_id` FOREIGN KEY (`original_bill_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill`(`medical_bill_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ADD CONSTRAINT `fk_claims_repair_estimate_approved_by_adjuster_id` FOREIGN KEY (`approved_by_adjuster_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`adjuster`(`adjuster_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ADD CONSTRAINT `fk_claims_repair_estimate_claim_exposure_id` FOREIGN KEY (`claim_exposure_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claim_exposure`(`claim_exposure_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ADD CONSTRAINT `fk_claims_repair_estimate_claim_id` FOREIGN KEY (`claim_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claim`(`claim_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ADD CONSTRAINT `fk_claims_repair_estimate_prior_estimate_repair_estimate_id` FOREIGN KEY (`prior_estimate_repair_estimate_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate`(`repair_estimate_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ADD CONSTRAINT `fk_claims_salvage_disposition_adjuster_id` FOREIGN KEY (`adjuster_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`adjuster`(`adjuster_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ADD CONSTRAINT `fk_claims_salvage_disposition_claim_exposure_id` FOREIGN KEY (`claim_exposure_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claim_exposure`(`claim_exposure_id`);
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ADD CONSTRAINT `fk_claims_salvage_disposition_claim_id` FOREIGN KEY (`claim_id`) REFERENCES `vibe_pc_insurance_blog_v499`.`claims`.`claim`(`claim_id`);
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`loss_event` SET TAGS ('dbx_subdomain' = 'loss_intake');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim` SET TAGS ('dbx_subdomain' = 'loss_intake');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim` ALTER COLUMN `inspection_order_id` SET TAGS ('dbx_business_glossary_term' = 'Inspection Order Id (Foreign Key)');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim` ALTER COLUMN `submission_id` SET TAGS ('dbx_business_glossary_term' = 'Submission Id (Foreign Key)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim` ALTER COLUMN `underwriting_risk_score_id` SET TAGS ('dbx_business_glossary_term' = 'Underwriting Risk Score Id (Foreign Key)');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim` ALTER COLUMN `lob_description` SET TAGS ('dbx_business_glossary_term' = 'Line of Business (LOB) Description');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim` ALTER COLUMN `peril_code` SET TAGS ('dbx_business_glossary_term' = 'Peril Code');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim` ALTER COLUMN `peril_description` SET TAGS ('dbx_business_glossary_term' = 'Peril Description');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` SET TAGS ('dbx_subdomain' = 'loss_intake');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_address_line1` SET TAGS ('dbx_business_glossary_term' = 'Mailing Address Line 1');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_address_line1` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_address_line1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_address_line2` SET TAGS ('dbx_business_glossary_term' = 'Mailing Address Line 2');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_address_line2` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_address_line2` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_city` SET TAGS ('dbx_business_glossary_term' = 'Mailing City');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_city` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_city` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_country_code` SET TAGS ('dbx_business_glossary_term' = 'Mailing Country Code');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_country_code` SET TAGS ('dbx_value_regex' = 'USA|CAN|MEX');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_country_code` SET TAGS ('dbx_pii_sensitive' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_postal_code` SET TAGS ('dbx_business_glossary_term' = 'Mailing Postal Code');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_postal_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_postal_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_state_province` SET TAGS ('dbx_business_glossary_term' = 'Mailing State or Province');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_state_province` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claimant` ALTER COLUMN `mailing_state_province` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_exposure` SET TAGS ('dbx_subdomain' = 'loss_intake');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_exposure` ALTER COLUMN `catastrophegeography_peril_id` SET TAGS ('dbx_business_glossary_term' = 'Peril Id (Foreign Key)');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_exposure` ALTER COLUMN `quote_coverage_id` SET TAGS ('dbx_business_glossary_term' = 'Quote Coverage Id (Foreign Key)');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_status` SET TAGS ('dbx_subdomain' = 'loss_intake');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_status` ALTER COLUMN `status_claim_id` SET TAGS ('dbx_business_glossary_term' = 'Claim ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_status` ALTER COLUMN `transaction_claim_id` SET TAGS ('dbx_business_glossary_term' = 'Transaction ID');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`adjuster` SET TAGS ('dbx_subdomain' = 'adjudication_management');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`adjuster` ALTER COLUMN `license_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'License Expiration Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`adjuster` ALTER COLUMN `license_issue_date` SET TAGS ('dbx_business_glossary_term' = 'License Issue Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`adjuster` ALTER COLUMN `license_number` SET TAGS ('dbx_business_glossary_term' = 'Adjuster License Number');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`adjuster` ALTER COLUMN `license_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`adjuster` ALTER COLUMN `license_number` SET TAGS ('dbx_pii_category' = 'government_id');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`adjuster` ALTER COLUMN `license_state` SET TAGS ('dbx_business_glossary_term' = 'License State');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`adjuster` ALTER COLUMN `license_state` SET TAGS ('dbx_pii_category' = 'government_id');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`adjuster_assignment` SET TAGS ('dbx_subdomain' = 'adjudication_management');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`fnol` SET TAGS ('dbx_subdomain' = 'loss_intake');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`fnol` ALTER COLUMN `submission_id` SET TAGS ('dbx_business_glossary_term' = 'Submission Id (Foreign Key)');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`fnol` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`fnol` ALTER COLUMN `lob` SET TAGS ('dbx_business_glossary_term' = 'Line of Business (LOB)');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`litigation` SET TAGS ('dbx_subdomain' = 'adjudication_management');
...
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` SET TAGS ('dbx_subdomain' = 'adjudication_management');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `siu_referral_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Key for siu_referral');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `adjuster_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Investigator ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `claim_id` SET TAGS ('dbx_business_glossary_term' = 'Claim ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `referring_party_id` SET TAGS ('dbx_business_glossary_term' = 'Referring Party ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `assignment_date` SET TAGS ('dbx_business_glossary_term' = 'Assignment Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `case_number` SET TAGS ('dbx_business_glossary_term' = 'Law Enforcement Case Number');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `clue_report_ordered_flag` SET TAGS ('dbx_business_glossary_term' = 'Comprehensive Loss Underwriting Exchange (CLUE) Report Ordered Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `confirmed_fraud_amount` SET TAGS ('dbx_business_glossary_term' = 'Confirmed Fraud Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `confirmed_fraud_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `disposition` SET TAGS ('dbx_business_glossary_term' = 'Disposition');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `estimated_fraud_amount` SET TAGS ('dbx_business_glossary_term' = 'Estimated Fraud Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `estimated_fraud_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `evidence_summary` SET TAGS ('dbx_business_glossary_term' = 'Evidence Summary');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `evidence_summary` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `external_database_check_flag` SET TAGS ('dbx_business_glossary_term' = 'External Database Check Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `fraud_indicator_code` SET TAGS ('dbx_business_glossary_term' = 'Fraud Indicator Code');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `fraud_indicator_description` SET TAGS ('dbx_business_glossary_term' = 'Fraud Indicator Description');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `fraud_scheme_category` SET TAGS ('dbx_business_glossary_term' = 'Fraud Scheme Category');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `fraud_type` SET TAGS ('dbx_business_glossary_term' = 'Fraud Type');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `fraud_type` SET TAGS ('dbx_value_regex' = 'hard_fraud|soft_fraud|opportunistic|organized|internal|external');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `investigation_close_date` SET TAGS ('dbx_business_glossary_term' = 'Investigation Close Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `investigation_notes` SET TAGS ('dbx_business_glossary_term' = 'Investigation Notes');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `investigation_notes` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `investigation_outcome` SET TAGS ('dbx_business_glossary_term' = 'Investigation Outcome');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `investigation_outcome` SET TAGS ('dbx_value_regex' = 'confirmed_fraud|suspected_fraud|cleared|inconclusive|withdrawn|referred_to_law_enforcement');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `investigation_start_date` SET TAGS ('dbx_business_glossary_term' = 'Investigation Start Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `iso_claim_search_flag` SET TAGS ('dbx_business_glossary_term' = 'Insurance Services Office (ISO) Claim Search Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `law_enforcement_agency` SET TAGS ('dbx_business_glossary_term' = 'Law Enforcement Agency');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `law_enforcement_referral_date` SET TAGS ('dbx_business_glossary_term' = 'Law Enforcement Referral Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `law_enforcement_referral_flag` SET TAGS ('dbx_business_glossary_term' = 'Law Enforcement Referral Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `naic_fraud_category_code` SET TAGS ('dbx_business_glossary_term' = 'National Association of Insurance Commissioners (NAIC) Fraud Category Code');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `nicb_referral_flag` SET TAGS ('dbx_business_glossary_term' = 'National Insurance Crime Bureau (NICB) Referral Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `policy_cancellation_recommended_flag` SET TAGS ('dbx_business_glossary_term' = 'Policy Cancellation Recommended Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Priority Level');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `priority_level` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `recovery_action_taken` SET TAGS ('dbx_business_glossary_term' = 'Recovery Action Taken');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `recovery_action_taken` SET TAGS ('dbx_value_regex' = 'none|subrogation|restitution|civil_suit|settlement|claim_denial');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `referral_date` SET TAGS ('dbx_business_glossary_term' = 'Referral Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `referral_number` SET TAGS ('dbx_business_glossary_term' = 'Special Investigations Unit (SIU) Referral Number');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `referral_source` SET TAGS ('dbx_business_glossary_term' = 'Referral Source');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `referral_status` SET TAGS ('dbx_business_glossary_term' = 'Referral Status');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `referral_status` SET TAGS ('dbx_value_regex' = 'pending|under_investigation|closed_confirmed_fraud|closed_cleared|closed_insufficient_evidence|withdrawn');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `referral_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Referral Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `referring_party_role` SET TAGS ('dbx_business_glossary_term' = 'Referring Party Role');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `savings_amount` SET TAGS ('dbx_business_glossary_term' = 'Savings Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `savings_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `statement_obtained_flag` SET TAGS ('dbx_business_glossary_term' = 'Statement Obtained Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `surveillance_conducted_flag` SET TAGS ('dbx_business_glossary_term' = 'Surveillance Conducted Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`siu_referral` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` SET TAGS ('dbx_subdomain' = 'loss_intake');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `claim_note_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Note Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `claim_author_party_id` SET TAGS ('dbx_business_glossary_term' = 'Author Party Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `claim_exposure_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Exposure Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `claim_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `claim_last_modified_by_party_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By Party Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `related_document_claim_document_id` SET TAGS ('dbx_business_glossary_term' = 'Related Document Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `activity_code` SET TAGS ('dbx_business_glossary_term' = 'Activity Code');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `attorney_client_privilege_flag` SET TAGS ('dbx_business_glossary_term' = 'Attorney-Client Privilege Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `author_name` SET TAGS ('dbx_business_glossary_term' = 'Author Name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `author_name` SET TAGS ('dbx_pii_category' = 'name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `author_role` SET TAGS ('dbx_business_glossary_term' = 'Author Role');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `confidential_flag` SET TAGS ('dbx_business_glossary_term' = 'Confidential Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `coverage_analysis_flag` SET TAGS ('dbx_business_glossary_term' = 'Coverage Analysis Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `diary_completed_flag` SET TAGS ('dbx_business_glossary_term' = 'Diary Completed Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `diary_completed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Diary Completed Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `diary_due_date` SET TAGS ('dbx_business_glossary_term' = 'Diary Due Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `diary_flag` SET TAGS ('dbx_business_glossary_term' = 'Diary Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `entry_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Entry Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `external_communication_flag` SET TAGS ('dbx_business_glossary_term' = 'External Communication Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `fraud_indicator_flag` SET TAGS ('dbx_business_glossary_term' = 'Fraud Indicator Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `liability_analysis_flag` SET TAGS ('dbx_business_glossary_term' = 'Liability Analysis Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `loss_description_flag` SET TAGS ('dbx_business_glossary_term' = 'Loss Description Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `note_category` SET TAGS ('dbx_business_glossary_term' = 'Note Category');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `note_number` SET TAGS ('dbx_business_glossary_term' = 'Note Number');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `note_source_system` SET TAGS ('dbx_business_glossary_term' = 'Note Source System');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `note_status` SET TAGS ('dbx_business_glossary_term' = 'Note Status');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `note_status` SET TAGS ('dbx_value_regex' = 'ACTIVE|ARCHIVED|DELETED|SUPERSEDED');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `note_subject` SET TAGS ('dbx_business_glossary_term' = 'Note Subject');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `note_text` SET TAGS ('dbx_business_glossary_term' = 'Note Text');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `note_text` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `note_type_code` SET TAGS ('dbx_business_glossary_term' = 'Note Type Code');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `regulatory_reportable_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reportable Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `reserve_recommendation_flag` SET TAGS ('dbx_business_glossary_term' = 'Reserve Recommendation Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `sequence_number` SET TAGS ('dbx_business_glossary_term' = 'Sequence Number');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `settlement_discussion_flag` SET TAGS ('dbx_business_glossary_term' = 'Settlement Discussion Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `visibility_classification` SET TAGS ('dbx_business_glossary_term' = 'Visibility Classification');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `visibility_classification` SET TAGS ('dbx_value_regex' = 'PUBLIC|INTERNAL|CONFIDENTIAL|RESTRICTED|LEGAL_PRIVILEGED');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_note` ALTER COLUMN `work_product_flag` SET TAGS ('dbx_business_glossary_term' = 'Work Product Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` SET TAGS ('dbx_subdomain' = 'loss_intake');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `claim_document_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Document Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `claim_exposure_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Exposure Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `claim_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `source_party_id` SET TAGS ('dbx_business_glossary_term' = 'Source Party Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `access_level` SET TAGS ('dbx_business_glossary_term' = 'Access Level');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `access_level` SET TAGS ('dbx_value_regex' = 'public|internal|restricted|confidential');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `document_category` SET TAGS ('dbx_business_glossary_term' = 'Document Category');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `document_date` SET TAGS ('dbx_business_glossary_term' = 'Document Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `document_description` SET TAGS ('dbx_business_glossary_term' = 'Document Description');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `document_number` SET TAGS ('dbx_business_glossary_term' = 'Document Number');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `document_status` SET TAGS ('dbx_business_glossary_term' = 'Document Status');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `document_title` SET TAGS ('dbx_business_glossary_term' = 'Document Title');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `document_type` SET TAGS ('dbx_business_glossary_term' = 'Document Type');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `file_format` SET TAGS ('dbx_business_glossary_term' = 'File Format');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `file_name` SET TAGS ('dbx_business_glossary_term' = 'File Name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `file_name` SET TAGS ('dbx_pii_category' = 'name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `file_size_bytes` SET TAGS ('dbx_business_glossary_term' = 'File Size in Bytes');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `is_confidential` SET TAGS ('dbx_business_glossary_term' = 'Is Confidential Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `is_required` SET TAGS ('dbx_business_glossary_term' = 'Is Required Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `is_verified` SET TAGS ('dbx_business_glossary_term' = 'Is Verified Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `page_count` SET TAGS ('dbx_business_glossary_term' = 'Page Count');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `received_date` SET TAGS ('dbx_business_glossary_term' = 'Received Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `received_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Received Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `retention_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Retention Expiration Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `retention_period_years` SET TAGS ('dbx_business_glossary_term' = 'Retention Period in Years');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `reviewed_by_user_code` SET TAGS ('dbx_business_glossary_term' = 'Reviewed By User Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `reviewed_by_user_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `reviewed_by_user_code` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `reviewed_date` SET TAGS ('dbx_business_glossary_term' = 'Reviewed Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `source_name` SET TAGS ('dbx_business_glossary_term' = 'Source Name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `source_name` SET TAGS ('dbx_pii_category' = 'name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `source_type` SET TAGS ('dbx_business_glossary_term' = 'Source Type');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `storage_location` SET TAGS ('dbx_business_glossary_term' = 'Storage Location');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `storage_system` SET TAGS ('dbx_business_glossary_term' = 'Storage System');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `storage_system` SET TAGS ('dbx_value_regex' = 'dms|ecm|cloud_storage|file_server|archive|imaging_system');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `verification_method` SET TAGS ('dbx_business_glossary_term' = 'Verification Method');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`claim_document` ALTER COLUMN `verification_method` SET TAGS ('dbx_value_regex' = 'manual_review|third_party_validation|digital_signature|notarization|cross_reference|none');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` SET TAGS ('dbx_subdomain' = 'recovery_operations');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `medical_bill_id` SET TAGS ('dbx_business_glossary_term' = 'Medical Bill Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `medical_bill_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `medical_bill_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `medical_bill_id` SET TAGS ('dbx_pii_category' = 'health');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `adjuster_id` SET TAGS ('dbx_business_glossary_term' = 'Adjuster Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `claim_exposure_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Exposure Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `claim_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `claimant_id` SET TAGS ('dbx_business_glossary_term' = 'Claimant Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `currency_id` SET TAGS ('dbx_business_glossary_term' = 'Currency Code (ISO 4217)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `original_bill_id` SET TAGS ('dbx_business_glossary_term' = 'Original Medical Bill Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `party_id` SET TAGS ('dbx_business_glossary_term' = 'Medical Provider Identifier (ID)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `allowed_amount` SET TAGS ('dbx_business_glossary_term' = 'Allowed Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `bill_date` SET TAGS ('dbx_business_glossary_term' = 'Bill Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `bill_number` SET TAGS ('dbx_business_glossary_term' = 'Medical Bill Number');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `bill_received_date` SET TAGS ('dbx_business_glossary_term' = 'Bill Received Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `bill_review_notes` SET TAGS ('dbx_business_glossary_term' = 'Bill Review Notes');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `bill_review_outcome` SET TAGS ('dbx_business_glossary_term' = 'Bill Review Outcome');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `bill_review_outcome` SET TAGS ('dbx_value_regex' = 'approved|reduced|denied|pending|not_reviewed');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `bill_source` SET TAGS ('dbx_business_glossary_term' = 'Bill Source');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `bill_source` SET TAGS ('dbx_value_regex' = 'provider_direct|claimant_submitted|attorney_submitted|third_party_administrator|electronic_data_interchange');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `bill_status` SET TAGS ('dbx_business_glossary_term' = 'Medical Bill Status');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `bill_type` SET TAGS ('dbx_business_glossary_term' = 'Medical Bill Type');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `billed_amount` SET TAGS ('dbx_business_glossary_term' = 'Billed Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `coinsurance_amount` SET TAGS ('dbx_business_glossary_term' = 'Coinsurance Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `copay_amount` SET TAGS ('dbx_business_glossary_term' = 'Copayment Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `deductible_amount` SET TAGS ('dbx_business_glossary_term' = 'Deductible Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `denial_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Denial Reason Code');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `denial_reason_description` SET TAGS ('dbx_business_glossary_term' = 'Denial Reason Description');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `diagnosis_code_primary` SET TAGS ('dbx_business_glossary_term' = 'Primary Diagnosis Code (ICD-10)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `diagnosis_code_primary` SET TAGS ('dbx_value_regex' = '^[A-Z][0-9]{2}(.[0-9]{1,4})?$');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `diagnosis_code_primary` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `diagnosis_code_primary` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `diagnosis_code_secondary` SET TAGS ('dbx_business_glossary_term' = 'Secondary Diagnosis Codes (ICD-10)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `diagnosis_code_secondary` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `diagnosis_code_secondary` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `is_duplicate` SET TAGS ('dbx_business_glossary_term' = 'Is Duplicate Bill Flag');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `paid_amount` SET TAGS ('dbx_business_glossary_term' = 'Paid Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `patient_dob` SET TAGS ('dbx_business_glossary_term' = 'Patient Date of Birth (DOB)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `patient_dob` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `patient_dob` SET TAGS ('dbx_pii_dob' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `patient_name` SET TAGS ('dbx_business_glossary_term' = 'Patient Name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `patient_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `patient_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `payment_date` SET TAGS ('dbx_business_glossary_term' = 'Payment Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `payment_method` SET TAGS ('dbx_business_glossary_term' = 'Payment Method');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `payment_method` SET TAGS ('dbx_value_regex' = 'check|eft|wire|card|other');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `payment_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Payment Reference Number');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `procedure_code_primary` SET TAGS ('dbx_business_glossary_term' = 'Primary Procedure Code (CPT)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `procedure_code_primary` SET TAGS ('dbx_value_regex' = '^[0-9]{5}$');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `procedure_code_secondary` SET TAGS ('dbx_business_glossary_term' = 'Secondary Procedure Codes (CPT)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `provider_name` SET TAGS ('dbx_business_glossary_term' = 'Medical Provider Name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `provider_name` SET TAGS ('dbx_pii_category' = 'name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `provider_npi` SET TAGS ('dbx_business_glossary_term' = 'National Provider Identifier (NPI)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `provider_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `provider_tax_number` SET TAGS ('dbx_business_glossary_term' = 'Provider Tax Identification Number (TIN)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `provider_tax_number` SET TAGS ('dbx_value_regex' = '^[0-9]{2}-[0-9]{7}$');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `provider_tax_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `reduction_amount` SET TAGS ('dbx_business_glossary_term' = 'Reduction Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `service_from_date` SET TAGS ('dbx_business_glossary_term' = 'Service From Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `service_to_date` SET TAGS ('dbx_business_glossary_term' = 'Service To Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`medical_bill` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` SET TAGS ('dbx_subdomain' = 'recovery_operations');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `repair_estimate_id` SET TAGS ('dbx_business_glossary_term' = 'Repair Estimate ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `approved_by_adjuster_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Adjuster ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `claim_exposure_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Exposure ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `claim_id` SET TAGS ('dbx_business_glossary_term' = 'Claim ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `currency_id` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `estimator_party_id` SET TAGS ('dbx_business_glossary_term' = 'Estimator Party Id (Foreign Key)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `prior_estimate_repair_estimate_id` SET TAGS ('dbx_business_glossary_term' = 'Prior Estimate ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `agreed_amount` SET TAGS ('dbx_business_glossary_term' = 'Agreed Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `betterment_amount` SET TAGS ('dbx_business_glossary_term' = 'Betterment Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `damage_description` SET TAGS ('dbx_business_glossary_term' = 'Damage Description');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `deductible_applied_amount` SET TAGS ('dbx_business_glossary_term' = 'Deductible Applied Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `depreciation_amount` SET TAGS ('dbx_business_glossary_term' = 'Depreciation Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `document_reference` SET TAGS ('dbx_business_glossary_term' = 'Document Reference');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `estimate_date` SET TAGS ('dbx_business_glossary_term' = 'Estimate Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `estimate_method` SET TAGS ('dbx_business_glossary_term' = 'Estimate Method');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `estimate_method` SET TAGS ('dbx_value_regex' = 'manual|software_assisted|photo_estimate|ai_estimate|field_inspection');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `estimate_notes` SET TAGS ('dbx_business_glossary_term' = 'Estimate Notes');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `estimate_number` SET TAGS ('dbx_business_glossary_term' = 'Estimate Number');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `estimate_status` SET TAGS ('dbx_business_glossary_term' = 'Estimate Status');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `estimate_status` SET TAGS ('dbx_value_regex' = 'draft|submitted|under_review|approved|rejected|superseded');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `estimate_type` SET TAGS ('dbx_business_glossary_term' = 'Estimate Type');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `estimate_type` SET TAGS ('dbx_value_regex' = 'initial|supplement|revised|final|desk_review|field_inspection');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `estimating_software` SET TAGS ('dbx_business_glossary_term' = 'Estimating Software');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `estimator_type` SET TAGS ('dbx_business_glossary_term' = 'Estimator Type');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `estimator_type` SET TAGS ('dbx_value_regex' = 'staff_adjuster|independent_adjuster|repair_shop|third_party_appraiser|insured_estimate');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `inspection_date` SET TAGS ('dbx_business_glossary_term' = 'Inspection Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `labor_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Labor Cost Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `labor_hours` SET TAGS ('dbx_business_glossary_term' = 'Labor Hours');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `labor_rate_per_hour` SET TAGS ('dbx_business_glossary_term' = 'Labor Rate Per Hour');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `net_payable_amount` SET TAGS ('dbx_business_glossary_term' = 'Net Payable Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `paint_materials_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Paint and Materials Cost Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `parts_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Parts Cost Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `parts_source` SET TAGS ('dbx_business_glossary_term' = 'Parts Source');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `parts_source` SET TAGS ('dbx_value_regex' = 'oem|aftermarket|recycled|refurbished|used');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `photo_count` SET TAGS ('dbx_business_glossary_term' = 'Photo Count');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `photo_count` SET TAGS ('dbx_pii_sensitive' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `repair_facility_address` SET TAGS ('dbx_business_glossary_term' = 'Repair Facility Address');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `repair_facility_address` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `repair_facility_address` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `repair_facility_name` SET TAGS ('dbx_business_glossary_term' = 'Repair Facility Name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `repair_facility_name` SET TAGS ('dbx_pii_category' = 'name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `repair_facility_phone` SET TAGS ('dbx_business_glossary_term' = 'Repair Facility Phone Number');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `repair_facility_phone` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `repair_facility_phone` SET TAGS ('dbx_pii_phone' = 'true');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `repair_facility_type` SET TAGS ('dbx_business_glossary_term' = 'Repair Facility Type');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `repair_facility_type` SET TAGS ('dbx_value_regex' = 'network_shop|preferred_vendor|oem_certified|independent_shop|contractor|restoration_company');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `repair_scope_description` SET TAGS ('dbx_business_glossary_term' = 'Repair Scope Description');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `sublet_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Sublet Cost Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `supplement_number` SET TAGS ('dbx_business_glossary_term' = 'Supplement Number');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `supplement_reason` SET TAGS ('dbx_business_glossary_term' = 'Supplement Reason');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `tax_amount` SET TAGS ('dbx_business_glossary_term' = 'Tax Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `total_estimate_amount` SET TAGS ('dbx_business_glossary_term' = 'Total Estimate Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`repair_estimate` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` SET TAGS ('dbx_subdomain' = 'recovery_operations');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `salvage_disposition_id` SET TAGS ('dbx_business_glossary_term' = 'Salvage Disposition ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `adjuster_id` SET TAGS ('dbx_business_glossary_term' = 'Adjuster ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `claim_exposure_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Exposure ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `claim_id` SET TAGS ('dbx_business_glossary_term' = 'Claim ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `currency_id` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `party_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `salvage_party_id` SET TAGS ('dbx_business_glossary_term' = 'Attorney ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `salvage_subrogation_target_party_id` SET TAGS ('dbx_business_glossary_term' = 'Subrogation Target Party ID');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `abandonment_reason` SET TAGS ('dbx_business_glossary_term' = 'Abandonment Reason');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `actual_salvage_value` SET TAGS ('dbx_business_glossary_term' = 'Actual Salvage Value');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `buyer_name` SET TAGS ('dbx_business_glossary_term' = 'Buyer Name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `buyer_name` SET TAGS ('dbx_pii_category' = 'name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `closed_date` SET TAGS ('dbx_business_glossary_term' = 'Disposition Closed Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `disposition_notes` SET TAGS ('dbx_business_glossary_term' = 'Disposition Notes');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `disposition_number` SET TAGS ('dbx_business_glossary_term' = 'Disposition Number');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `disposition_status` SET TAGS ('dbx_business_glossary_term' = 'Disposition Status');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `disposition_status` SET TAGS ('dbx_value_regex' = 'open|pending|closed|abandoned|settled|in_litigation');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `estimated_salvage_value` SET TAGS ('dbx_business_glossary_term' = 'Estimated Salvage Value');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `litigation_filed_date` SET TAGS ('dbx_business_glossary_term' = 'Litigation Filed Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `litigation_status` SET TAGS ('dbx_business_glossary_term' = 'Litigation Status');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `litigation_status` SET TAGS ('dbx_value_regex' = 'not_applicable|pending|filed|settled|judgment|dismissed');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `net_recovery_amount` SET TAGS ('dbx_business_glossary_term' = 'Net Recovery Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `opened_date` SET TAGS ('dbx_business_glossary_term' = 'Disposition Opened Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `recovery_expenses` SET TAGS ('dbx_business_glossary_term' = 'Recovery Expenses');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `recovery_type` SET TAGS ('dbx_business_glossary_term' = 'Recovery Type');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `recovery_type` SET TAGS ('dbx_value_regex' = 'salvage|subrogation|reinsurance|other');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `sale_date` SET TAGS ('dbx_business_glossary_term' = 'Salvage Sale Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `sale_method` SET TAGS ('dbx_business_glossary_term' = 'Sale Method');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `sale_method` SET TAGS ('dbx_value_regex' = 'auction|private_sale|scrap|donation|retained|other');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `salvage_item_description` SET TAGS ('dbx_business_glossary_term' = 'Salvage Item Description');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `salvage_item_type` SET TAGS ('dbx_business_glossary_term' = 'Salvage Item Type');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `salvage_item_type` SET TAGS ('dbx_value_regex' = 'vehicle|building_materials|equipment|inventory|personal_property|other');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `settlement_date` SET TAGS ('dbx_business_glossary_term' = 'Settlement Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `storage_end_date` SET TAGS ('dbx_business_glossary_term' = 'Storage End Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `storage_location` SET TAGS ('dbx_business_glossary_term' = 'Storage Location');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `storage_start_date` SET TAGS ('dbx_business_glossary_term' = 'Storage Start Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `subrogation_demand_amount` SET TAGS ('dbx_business_glossary_term' = 'Subrogation Demand Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `subrogation_recovered_amount` SET TAGS ('dbx_business_glossary_term' = 'Subrogation Recovered Amount');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `subrogation_target_name` SET TAGS ('dbx_business_glossary_term' = 'Subrogation Target Name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `subrogation_target_name` SET TAGS ('dbx_pii_category' = 'name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `title_status` SET TAGS ('dbx_business_glossary_term' = 'Title Status');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `title_status` SET TAGS ('dbx_value_regex' = 'clean|salvage|rebuilt|junk|pending|not_applicable');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `title_transfer_date` SET TAGS ('dbx_business_glossary_term' = 'Title Transfer Date');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `vehicle_vin` SET TAGS ('dbx_business_glossary_term' = 'Vehicle Identification Number (VIN)');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `vehicle_vin` SET TAGS ('dbx_value_regex' = '^[A-HJ-NPR-Z0-9]{17}$');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `vendor_name` SET TAGS ('dbx_business_glossary_term' = 'Vendor Name');
ALTER TABLE `vibe_pc_insurance_blog_v499`.`claims`.`salvage_disposition` ALTER COLUMN `vendor_name` SET TAGS ('dbx_pii_category' = 'name');
