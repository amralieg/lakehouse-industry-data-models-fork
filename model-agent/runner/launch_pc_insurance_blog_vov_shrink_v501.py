#!/usr/bin/env python3
"""Shrink the VoV-produced v2 P&C ECM to v2 MVM on agent v5.0.1.

Reads the completed v2 ECM in `vibe_pc_insurance_blog_v499` (the output of
launch_pc_insurance_blog_vov_v500.py: claims->claim with claim_financials folded in
as a subdomain, risk_exposure->risk, catastrophe_geography->catastrophe) and produces
the MVM scope at the SAME version 2. The eleven renamed domains are the §3b SSOT; the
shrink keeps the FK-densest survivor products so the MVM stays a connected core.
"""
import json
import os
import sys
...
PROFILE = "my-uae"
AGENT_PATH = "/Users/user@example.com/dbx_vibe_modelling_agent_v501"
...
M.PULSE_FILE = os.path.expanduser("~/claude/vibe-agent/pc_insurance_blog_vov_shrink_v501_pulses.txt")
M.STATE_FILE = os.path.expanduser("~/claude/vibe-agent/pc_insurance_blog_vov_shrink_v501_state.json")
M.KILL_FILE = os.path.expanduser("~/claude/vibe-agent/pc_insurance_blog_vov_shrink_v501_KILL")
M.PULSE_S = 300
...
def build_spec():
    params = {
        ...
        "model_version": "2",                     # read v2 ECM -> write v2 MVM
...
def find_or_create_job(spec):
    ...
    for job in items:
        if (job.get("settings", {}) or {}).get("name") == spec["name"]:
            Path("/tmp/pc_blog_vov_shrink_v501_jobpatch.json").write_text(
                json.dumps({"job_id": job["job_id"], "new_settings": spec}))
            M.db(["jobs", "reset", "--json", "@/tmp/pc_blog_vov_shrink_v501_jobpatch.json"], PROFILE)
            return job["job_id"]
    Path("/tmp/pc_blog_vov_shrink_v501_jobspec.json").write_text(json.dumps(spec))
    return M.dbj(["jobs", "create", "--json", "@/tmp/pc_blog_vov_shrink_v501_jobspec.json"], PROFILE)["job_id"]


def info_log_bytes():
    base = "dbfs:/Volumes/%s/_metamodel/vol_root/logs/%s/v2/mvm" % (CATALOG, BUSINESS)
    try:
...
def main():
    ...
    M.pulse("=== P&C BLOG VoV-SHRINK v2 ECM->MVM v5.0.1 START catalog=%s agent=%s ===" % (CATALOG, AGENT_PATH))
    ...
    Path("/tmp/pc_blog_vov_shrink_v501_run.json").write_text(json.dumps(state, indent=2))
    ...
    Path("/tmp/pc_blog_vov_shrink_v501_run.json").write_text(json.dumps(state, indent=2, default=str))
