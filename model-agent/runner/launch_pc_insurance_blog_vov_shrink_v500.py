#!/usr/bin/env python3
"""Shrink the VoV-produced v2 P&C ECM to v2 MVM on agent v5.0.0.
...
import launch_pc_insurance_blog_vov_v500 as VOV
...
M.PULSE_FILE = os.path.expanduser("~/claude/vibe-agent/pc_insurance_blog_vov_shrink_v500_pulses.txt")
M.STATE_FILE = os.path.expanduser("~/claude/vibe-agent/pc_insurance_blog_vov_shrink_v500_state.json")
M.KILL_FILE = os.path.expanduser("~/claude/vibe-agent/pc_insurance_blog_vov_shrink_v500_KILL")
M.PULSE_S = 300
...
def find_or_create_job(spec):
    ...
    for job in items:
        if (job.get("settings", {}) or {}).get("name") == spec["name"]:
            Path("/tmp/pc_blog_vov_shrink_v500_jobpatch.json").write_text(
                json.dumps({"job_id": job["job_id"], "new_settings": spec}))
            M.db(["jobs", "reset", "--json", "@/tmp/pc_blog_vov_shrink_v500_jobpatch.json"], PROFILE)
            return job["job_id"]
    Path("/tmp/pc_blog_vov_shrink_v500_jobspec.json").write_text(json.dumps(spec))
    return M.dbj(["jobs", "create", "--json", "@/tmp/pc_blog_vov_shrink_v500_jobspec.json"], PROFILE)["job_id"]
...
def main():
    ...
    M.pulse("=== P&C BLOG VoV-SHRINK v2 ECM->MVM v5.0.0 START catalog=%s agent=%s ===" % (CATALOG, AGENT_PATH))
    ...
    Path("/tmp/pc_blog_vov_shrink_v500_run.json").write_text(json.dumps(state, indent=2))
    ...
    Path("/tmp/pc_blog_vov_shrink_v500_run.json").write_text(json.dumps(state, indent=2, default=str))
